...
func TestUserValidationNegative(t *testing.T) {
	...
	t.Run("Email is empty", func(t *testing.T) {
		user := &User{Email: "", EncryptedPassword: "Password@123"}
	...
	t.Run("Email is invalid", func(t *testing.T) {
		user := &User{Email: "not_an_email", EncryptedPassword: "Password@123"}
		err := user.Validate(db)
		assert.Error(t, err)
	...
	t.Run("Email already exists", func(t *testing.T) {
		existingUser := &User{Email: "test@test.com", EncryptedPassword: "Password@123"}
		err := db.Create(existingUser).Error
		assert.NoError(t, err)

		newUser := &User{Email: "test@test.com", EncryptedPassword: "Password@123"}
	...
	t.Run("Password is too short", func(t *testing.T) {
		user := &User{Email: "test2@test.com", EncryptedPassword: "Short1!"}
		err := user.Validate(db)
		assert.Error(t, err)
	...
	t.Run("Password is too long", func(t *testing.T) {
		...
		user := &User{Email: "test3@test.com", EncryptedPassword: longPassword}
		err := user.Validate(db)
		assert.Error(t, err)
	...
	t.Run("Password does not meet complexity requirements", func(t *testing.T) {
		user := &User{Email: "test4@test.com", EncryptedPassword: "simplepassword"}
		err := user.Validate(db)
		assert.Error(t, err)
...
func TestUserValidationPositive(t *testing.T) {
	...
	t.Run("Valid User", func(t *testing.T) {
		user := &User{Email: "test5@test.com", EncryptedPassword: "Password@123"}
		err := user.Validate(db)
		assert.NoError(t, err)
	})

	t.Run("Create Valid User", func(t *testing.T) {
		user := &User{Email: "test6@test.com", EncryptedPassword: "Password@123"}
		passwordBeforeHash := user.EncryptedPassword
		err := user.Create(db)
		assert.NoError(t, err)
		...
		err = bcrypt.CompareHashAndPassword([]byte(dbUser.EncryptedPassword), []byte(passwordBeforeHash))
		assert.NoError(t, err)
	})
